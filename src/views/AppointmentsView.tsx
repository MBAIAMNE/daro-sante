...
export const AppointmentsView: React.FC = () => {
  ...
  return (
    <div className="p-4 sm:p-8 max-w-7xl mx-auto space-y-6">
      ...
      {/* Live Teleconsultation Room (WebRTC HD Real Audio & Video) */}
      {activeCallAppointment && (
        <TeleconsultationModal
          ...
          patient={
            patients.find(p => p.id === activeCallAppointment.patientId) || {
              id: activeCallAppointment.patientId,
              nom: activeCallAppointment.patientNom.split(' ').slice(1).join(' ') || activeCallAppointment.patientNom,
              prenom: activeCallAppointment.patientNom.split(' ')[0] || '',
              matricule: 'PAT-TELE',
              ...
              allergies: [],
              antecedents: [],
              contactUrgenceNom: 'Proche',
              contactUrgenceTelephone: '+235 66 00 00 00',
              contactUrgenceLien: 'Famille',
              etablissementId: currentUser.etablissementId || 'etab-1',
              dateCreation: new Date().toISOString().split('T')[0],
              statut: 'actif',
            }
          }
          doctorNom={activeCallAppointment.medecinNom || `Dr. ${currentUser.prenom} ${currentUser.nom}`}
          clinicNom={currentUser.etablissementNom || 'Réseau DARÔ Santé'}
          isDoctorMode={true}
        />
      )}
